...
class Commands:
    ...
    def short_help():
        ...
        return ', '.join([f'{k.upper()}={Display.term.red(Commands.CMDS[k]["short"])}' for k in Commands.CMDS.keys() if Commands.CMDS[k]["short"]])

    def long_help():
        """Long help - a one-pager"""
        return '\n'.join([f'{k.upper()} - {Commands.CMDS[k]["long"]}' for k in Commands.CMDS.keys() if Commands.CMDS[k]["long"]])
